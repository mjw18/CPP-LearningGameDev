...
void BallController::updateBalls(std::vector <Ball>& balls, float deltaTime, int maxX, int maxY, Grid* grid) {  
    ...
    for (size_t i = 0; i < balls.size(); i++) {
		...
		//check to see if ball changed cells
...
{
	for (int i = 0; i < grid->mCells.size(); i++)
	{
		int x = i % grid->mNumCellsX;
		int y = i / grid->mNumCellsX;

		Cell& cell = grid->mCells[i];
		...
		for (int j = 0; j < cell.balls.size(); j++)
		{
			...
			//update collisions with neighbors
			if (x > 0)
			{
				if (y > 0)
				{
...
void BallController::checkCollision(Ball& b1, Ball& b2) {
    // We use the center of the ball as position
