...
MainGame::~MainGame()
{
	for (int i = 0; i < mBallRenderers.size(); i++)
	{
		delete mBallRenderers[i];
	}
}
...
void MainGame::init() {
    ...
    m_window.createWindow("Ball Game", m_screenWidth, m_screenHeight, 0);
    ...
    m_spriteFont = std::make_unique<WebEngine::SpriteFont>("Fonts/chintzy.ttf", 40);
...
{
	mBallRenderers.push_back(new BallRenderer);
	mBallRenderers.push_back(new MomentumBallRenderer);
}


struct BallSpawn {
    BallSpawn(const WebEngine::ColorRGBA8& colr,
...
void MainGame::initBalls() {
	...
	//Initialize grid with unique ptr
	mGrid = std::make_unique<Grid>(m_screenWidth, m_screenHeight, CELL_SIZE);
...
#define ADD_BALL(p, ...) \
    ...
    ADD_BALL(10.0f, WebEngine::ColorRGBA8(255, 255, 255, 255),
             1.0f, 1.0f, 0.1f, 7.0f, totalProbability);
    ADD_BALL(40.0f, WebEngine::ColorRGBA8(0, 0, 255, 255),
             1.0f, 2.0f, 0.1f, 3.0f, totalProbability);
    ADD_BALL(1.0f, WebEngine::ColorRGBA8(255, 0, 0, 255),
             2.0f, 4.0f, 0.0f, 0.0f, totalProbability)
    ...
    for (int i = 0; i < NUM_BALLS; i++) {
        ...
        // Add ball
        m_balls.emplace_back(ballToSpawn->radius, ballToSpawn->mass, pos, direction * ballToSpawn->randSpeed(randomEngine)* 0.1f,
                             WebEngine::ResourceManager::getTexture("Textures/circle.png").id,
                             ballToSpawn->color);
		//Dont call emplace back after this
		mGrid->addBall(&m_balls.back());
    }
}

void MainGame::update(float deltaTime) {
    m_ballController.updateBalls(m_balls, deltaTime, m_screenWidth, m_screenHeight, mGrid.get());
}
...
void MainGame::draw() {
    ...
    // Draw balls
    mBallRenderers[mCurrentRenderer]->renderBalls(m_spriteBatch, m_balls, projectionMatrix);
...
void MainGame::drawHud() {
    const WebEngine::ColorRGBA8 fontColor(255, 0, 0, 255);
...
void MainGame::processInput() {
	...
	{
		mCurrentRenderer++;
		if (mCurrentRenderer >= mBallRenderers.size())
		{
			mCurrentRenderer = 0;
		}
	}
